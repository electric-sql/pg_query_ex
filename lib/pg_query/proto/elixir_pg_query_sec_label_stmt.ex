...
defmodule PgQuery.SecLabelStmt do
  @moduledoc false
  defstruct objtype: :OBJECT_TYPE_UNDEFINED, object: nil, provider: "", label: ""
      ...
      defp encode_provider(acc, msg) do
        try do
          if msg.provider == "" do
            ...
            [acc, "\x1A", Protox.Encode.encode_string(msg.provider)]
          end
        rescue
          ArgumentError ->
      ...
      defp encode_label(acc, msg) do
        try do
          if msg.label == "" do
            ...
            [acc, "\"", Protox.Encode.encode_string(msg.label)]
          end
        rescue
          ArgumentError ->
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(PgQuery.SecLabelStmt))
        end
      )
    )
        ...
        {field, rest} =
          case Protox.Decode.parse_key(bytes) do
            ...
            {3, _, bytes} ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[provider: delimited], rest}

            {4, _, bytes} ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[label: delimited], rest}
