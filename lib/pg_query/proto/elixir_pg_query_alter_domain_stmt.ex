...
defmodule PgQuery.AlterDomainStmt do
      ...
      defp encode_subtype(acc, msg) do
        try do
          if msg.subtype == "" do
            ...
            [acc, "\n", Protox.Encode.encode_string(msg.subtype)]
          end
        rescue
          ArgumentError ->
      ...
      defp encode_missing_ok(acc, msg) do
        try do
          if msg.missing_ok == false do
            ...
            [acc, "0", Protox.Encode.encode_bool(msg.missing_ok)]
          end
        rescue
          ArgumentError ->
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(PgQuery.AlterDomainStmt))
        end
      )
    )
  ...
  (
    ...
    def defs() do
      %{
        1 => {:subtype, {:scalar, ""}, :string},
        2 => {:type_name, :unpacked, {:message, PgQuery.Node}},
        3 => {:name, {:scalar, ""}, :string},
        4 => {:def, {:scalar, nil}, {:message, PgQuery.Node}},
        5 => {:behavior, {:scalar, :DROP_BEHAVIOR_UNDEFINED}, {:enum, PgQuery.DropBehavior}},
        6 => {:missing_ok, {:scalar, false}, :bool}
      }
    end
    ...
    def defs_by_name() do
      %{
        behavior: {5, {:scalar, :DROP_BEHAVIOR_UNDEFINED}, {:enum, PgQuery.DropBehavior}},
        def: {4, {:scalar, nil}, {:message, PgQuery.Node}},
        missing_ok: {6, {:scalar, false}, :bool},
        name: {3, {:scalar, ""}, :string},
        subtype: {1, {:scalar, ""}, :string},
        type_name: {2, :unpacked, {:message, PgQuery.Node}}
      }
    end
  )
