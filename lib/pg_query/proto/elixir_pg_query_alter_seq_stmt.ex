...
defmodule PgQuery.AlterSeqStmt do
  @moduledoc false
  defstruct sequence: nil, options: [], for_identity: false, missing_ok: false
  ...
  (
    ...
    [
      defp encode_sequence(acc, msg) do
        try do
          if msg.sequence == nil do
            acc
          else
            [acc, "\n", Protox.Encode.encode_message(msg.sequence)]
          end
        rescue
          ArgumentError ->
      ...
      defp encode_for_identity(acc, msg) do
        try do
          if msg.for_identity == false do
            acc
          else
            [acc, "\x18", Protox.Encode.encode_bool(msg.for_identity)]
          end
        rescue
          ArgumentError ->
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(PgQuery.AlterSeqStmt))
        end
      )
    )
  ...
  (
    ...
    def defs() do
      %{
        1 => {:sequence, {:scalar, nil}, {:message, PgQuery.RangeVar}},
        2 => {:options, :unpacked, {:message, PgQuery.Node}},
        3 => {:for_identity, {:scalar, false}, :bool},
    ...
    def defs_by_name() do
      %{
        for_identity: {3, {:scalar, false}, :bool},
        missing_ok: {4, {:scalar, false}, :bool},
        options: {2, :unpacked, {:message, PgQuery.Node}},
        sequence: {1, {:scalar, nil}, {:message, PgQuery.RangeVar}}
      }
    end
  )
