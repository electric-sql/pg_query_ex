...
defmodule PgQuery.SQLValueFunction do
  @moduledoc false
  defstruct xpr: nil, op: :SQLVALUE_FUNCTION_OP_UNDEFINED, type: 0, typmod: 0, location: 0
  ...
  (
    ...
    [
      ...
      defp encode_op(acc, msg) do
        try do
          if msg.op == :SQLVALUE_FUNCTION_OP_UNDEFINED do
            acc
          else
            [
              acc,
              "\x10",
              msg.op |> PgQuery.SQLValueFunctionOp.encode() |> Protox.Encode.encode_enum()
            ]
          end
        rescue
          ArgumentError ->
      ...
      defp encode_typmod(acc, msg) do
        try do
          if msg.typmod == 0 do
            ...
            [acc, " ", Protox.Encode.encode_int32(msg.typmod)]
          end
        rescue
          ArgumentError ->
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(PgQuery.SQLValueFunction))
        end
      )
    )
      ...
      defp parse_key_value(bytes, msg) do
        {field, rest} =
          case Protox.Decode.parse_key(bytes) do
            ...
            {2, _, bytes} ->
              {value, rest} = Protox.Decode.parse_enum(bytes, PgQuery.SQLValueFunctionOp)
              {[op: value], rest}
  ...
  (
    ...
    def defs() do
      %{
        ...
        2 =>
          {:op, {:scalar, :SQLVALUE_FUNCTION_OP_UNDEFINED}, {:enum, PgQuery.SQLValueFunctionOp}},
        3 => {:type, {:scalar, 0}, :uint32},
        4 => {:typmod, {:scalar, 0}, :int32},
    ...
    def defs_by_name() do
      %{
        ...
        op: {2, {:scalar, :SQLVALUE_FUNCTION_OP_UNDEFINED}, {:enum, PgQuery.SQLValueFunctionOp}},
        type: {3, {:scalar, 0}, :uint32},
        typmod: {4, {:scalar, 0}, :int32},
    ...
    def fields_defs() do
      [
        ...
        %{
          __struct__: Protox.Field,
          json_name: "op",
          kind: {:scalar, :SQLVALUE_FUNCTION_OP_UNDEFINED},
          label: :optional,
          name: :op,
          tag: 2,
          type: {:enum, PgQuery.SQLValueFunctionOp}
        },
        %{
          __struct__: Protox.Field,
          json_name: "type",
  ...
  [
    ...
    def default(:op) do
      {:ok, :SQLVALUE_FUNCTION_OP_UNDEFINED}
    end,
    def default(:type) do
      {:ok, 0}
    end,
