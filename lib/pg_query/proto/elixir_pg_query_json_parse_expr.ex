...
defmodule PgQuery.JsonParseExpr do
  @moduledoc false
  defstruct expr: nil, output: nil, unique_keys: false, location: 0
      ...
      defp encode_unique_keys(acc, msg) do
        try do
          if msg.unique_keys == false do
            ...
            [acc, "\x18", Protox.Encode.encode_bool(msg.unique_keys)]
          end
        rescue
          ArgumentError ->
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(PgQuery.JsonParseExpr))
        end
      )
    )
        ...
        {field, rest} =
          case Protox.Decode.parse_key(bytes) do
            ...
            {1, _, bytes} ->
              ...
              {[
                 expr:
                   Protox.MergeMessage.merge(msg.expr, PgQuery.JsonValueExpr.decode!(delimited))
               ], rest}
  ...
  (
    ...
    def defs() do
      %{
        1 => {:expr, {:scalar, nil}, {:message, PgQuery.JsonValueExpr}},
        2 => {:output, {:scalar, nil}, {:message, PgQuery.JsonOutput}},
        3 => {:unique_keys, {:scalar, false}, :bool},
    ...
    def defs_by_name() do
      %{
        expr: {1, {:scalar, nil}, {:message, PgQuery.JsonValueExpr}},
        location: {4, {:scalar, 0}, :int32},
        output: {2, {:scalar, nil}, {:message, PgQuery.JsonOutput}},
        unique_keys: {3, {:scalar, false}, :bool}
      }
    end
  )
