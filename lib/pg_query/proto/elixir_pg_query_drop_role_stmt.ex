...
defmodule PgQuery.DropRoleStmt do
  @moduledoc false
  defstruct roles: [], missing_ok: false
  ...
  (
    (
      ...
      def encode!(msg) do
        [] |> encode_roles(msg) |> encode_missing_ok(msg)
      end
    )
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(PgQuery.DropRoleStmt))
        end
      )
    )
  ...
  (
    ...
    def defs_by_name() do
      %{missing_ok: {2, {:scalar, false}, :bool}, roles: {1, :unpacked, {:message, PgQuery.Node}}}
    end
  )
